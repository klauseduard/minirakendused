...
export function initJournal() {
    ...
    // FAB directly opens new entry (no sub-actions)
    if (fabBtn) {
    ...
    // Show/hide FAB based on journal visibility and viewport
    const journalSection = document.getElementById('garden-journal');
    function updateFabVisibility() {
        if (!fabBtn || !journalSection) return;
        const isJournalVisible = journalSection.style.display !== 'none';
        fabBtn.style.display = (isMobile() && isJournalVisible) ? 'flex' : 'none';
    }
    if (journalSection) {
